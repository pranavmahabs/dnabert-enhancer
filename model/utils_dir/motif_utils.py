...
def motif_analysis(
    ...
    **kwargs -- other input arguments, may include:
    ...
    for i, score in enumerate(pos_atten_scores):
        ...
        if "atten_cond" in kwargs:
            motif_regions = find_high_attention(
                score, min_len=min_len, cond=kwargs["atten_cond"]
            )
        else:
        ...
        for motif_idx in motif_regions:
            ...
            if seq not in motif_seqs:
                motif_seqs[seq] = {
                    "seq_idx": [i],
                    "atten_region_pos": [(motif_idx[0], motif_idx[1])],
                }
            else:
    ...
    if "align_cond" in kwargs:
        ...
        merged_motif_seqs = merge_motifs(
            motif_seqs, min_len=min_len, align_all_ties=align_all_ties
        )

    # make fixed-length window sequences
    if verbose:
        print("* Making fixed_length window = {}".format(window_size))
    merged_motif_seqs = make_window(
    ...
    merged_motif_seqs = {
        k: coords
        for k, coords in merged_motif_seqs.items()
    ...
    if save_file_dir is not None:
        ...
        for motif, instances in merged_motif_seqs.items():
            ...
            with open(
                save_file_dir
                + "/motif_{}_{}.txt".format(motif, len(instances["seq_idx"])),
                "w",
            ) as f:
            ...
            m.weblogo(
                save_file_dir
                + "/motif_{}_{}_weblogo.png".format(motif, len(instances["seq_idx"])),
                format="png_print",
