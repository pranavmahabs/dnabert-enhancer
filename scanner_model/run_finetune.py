...
def train(args, train_dataset, model, tokenizer):
    ...
    warmup_steps = (
        ...
        else int(args.warmup_percent * t_total)
    )
...
def evaluate(args, model, tokenizer, prefix="", evaluate=True):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        eval_dataset = load_and_cache_examples(
            args, eval_task, tokenizer, evaluate=evaluate
        )
        ...
        if args.output_mode == "classification":
            if args.task_name[:3] == "dna" and args.task_name != "dnasplice":
                if args.do_ensemble_pred:
                    ...
                    probs = softmax(torch.tensor(preds, dtype=torch.float32))[
                        :, 1
                    ].numpy()
...
def predict(args, model, tokenizer, prefix=""):
    ...
    for pred_task, pred_output_dir in zip(pred_task_names, pred_outputs_dirs):
        pred_dataset = load_and_cache_examples(
            args, pred_task, tokenizer, evaluate=True
        )
...
def visualize(args, model, tokenizer, kmer, prefix=""):
    ...
    for pred_task, pred_output_dir in zip(pred_task_names, pred_outputs_dirs):
        ...
        pred_dataset = load_and_cache_examples(
            args, pred_task, tokenizer, evaluate=evaluate
        )
        ...
        for index, batch in enumerate(tqdm(pred_dataloader, desc="Predicting")):
            ...
            with torch.no_grad():
                ...
                preds[index * batch_size : index * batch_size + len(batch[0]), :] = (
                    logits.detach().cpu().numpy()
                )
                attention_scores[
                    index * batch_size : index * batch_size + len(batch[0]), :, :, :
                ] = attention.cpu().numpy()
                # if preds is None:
                #     preds = logits.detach().cpu().numpy()
                # else:
...
def main():
    ...
    parser.add_argument(
        "--beta1", default=0.9, type=float, help="Beta1 for Adam optimizer."
    )
    parser.add_argument(
        "--beta2", default=0.999, type=float, help="Beta2 for Adam optimizer."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--rnn", default="lstm", type=str, help="What kind of RNN to use"
    )
    parser.add_argument(
        "--num_rnn_layer",
        default=2,
    ...
    if args.do_visualize and args.local_rank in [-1, 0]:
        visualization_models = (
            [3, 4, 5, 6] if not args.visualize_models else [args.visualize_models]
        )
        ...
        for kmer in visualization_models:
            ...
            model.to(args.device)
            attention_scores, probs = visualize(
                args, model, tokenizer, prefix=prefix, kmer=kmer
            )
    ...
    if args.do_ensemble_pred and args.local_rank in [-1, 0]:
        ...
        ensemble_results = compute_metrics(
            eval_task, all_preds, out_label_ids, all_probs[:, 1]
        )
